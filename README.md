...
Tech Stack
LayerChoiceFrameworkReact (hooks only, no class components)StateuseState — no external state libraryStylingCSS-in-JS via injected <style> tag with CSS custom propertiesTypographyGoogle Fonts — Cormorant Garamond + Instrument SansDataStatic in-memory constants (no API calls)RoutingConditional rendering (screen state variable)
...
bash# Install dependencies (if you don't have a React project already)
...
Post
js{ id, emoji, name, handle, time, text, nowPlaying: { emoji, song, artist } | null,
  likes, replies, reposts, liked, playing }
Fan
js{ id, emoji, name, handle, tags, bio, compat, cold: [4 strings],
  topAlbums: [{ emoji, title, artist, bg }], playing, nowSong }
Show
js{ id, emoji, artist, venue, date, going,
...
Spotify Web API — replace fans[].nowSong and INITIAL_POSTS[].nowPlaying with real listening data via the Spotify Web API
...
README Written by Claude Sonnet 4.6