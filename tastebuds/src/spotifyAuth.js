...
export async function fetchSpotifyTopTracks(accessToken, limit = 50) {
  const res = await fetch(
...
// Spotify has no "top albums" endpoint, so we derive it: take the user's top
// tracks (already ranked by listening) and dedupe down to unique albums,
// preserving rank order.
export async function fetchSpotifyTopAlbums(accessToken, limit = 4) {
